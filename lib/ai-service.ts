...
export class AIService {
  ...
  static async generateResume(prompt: string, selectedModel?: string): Promise<AIServiceResponse> {
    ...
    try {
      ...
      } else if (aiService === 'kimi') {
        return await this.callKimi(prompt);
      } else {
        return {
          success: false,
          error: 'Invalid AI service configuration. Supported models: "gemini" or "kimi".'
        };
      }
    } catch (error) {
  ...
  private static async callKimi(prompt: string): Promise<AIServiceResponse> {
    ...
    if (!content) {
      return {
        success: false,
        error: 'No content generated by Kimi K2 API'
      };
    }
