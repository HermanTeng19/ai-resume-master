...
export class AIService {
  ...
  private static async callDeepSeek(prompt: string): Promise<AIServiceResponse> {
        ...
        if (response.ok) {
          ...
          if (content) {
            return {
              success: true,
              content: content.trim(),
              model: `DeepSeek (${model.split('/')[1]}) via OpenRouter`
            };
          }
        } else {
          const errorData = await response.json().catch(() => ({}));
          lastError = `${model}: ${response.status} ${response.statusText} - ${errorData.error?.message || 'Unknown error'}`;
          console.warn(`Model ${model} failed, trying next...`, lastError);
        }
      } catch (error) {
        lastError = `${model}: ${error}`;
        console.warn(`Model ${model} failed with exception, trying next...`, error);
      }
    }
    ...
    return {
      success: false,
      error: `OpenRouter API error - all models failed. Last error: ${lastError}`
    };
  }
} 